...
const LIMIT_402G = 24500;           // standard elective deferral limit
const LIMIT_CATCHUP_50 = 8000;      // catch-up ages 50-59 and 64+
const LIMIT_CATCHUP_6063 = 11250;   // enhanced catch-up ages 60-63

// 415(c) annual additions limit (applies to 403(b) and 401(a) separately)
const LIMIT_415C = 72000;

// 457(b) limit (employee pre-tax only, no catch-up for this plan)
const LIMIT_457B = 24500;

// Employer contribution rates for 401(a) — update if plan changes
const MATCH_RATE = 0.50;            // 50% employer match
const MATCH_CAP_PCT = 0.08;         // on first 8% of salary
const DISCRETIONARY_RATE = 0.03;    // 3% employer discretionary

// Compensation basis cap (IRC 401(a)(17))
const COMP_LIMIT = 360000;

// ── Baptist Health 2026 Payroll Calendar ──────────────────────────────────────
// Checks Issued column = payday. Update each year with official calendar.
...
// Cutoff: Thursday at 10:00 AM Central Time (CT = UTC-6 standard, UTC-5 DST)
// Arkansas observes CDT (UTC-5) Mar–Nov, CST (UTC-6) Nov–Mar
const CUTOFF_HOUR_CT = 10;

function getCentralOffset(date) {
  // Arkansas uses CDT (UTC-5) from second Sunday in March to first Sunday in November
  ...
  return isDST ? -5 * 60 : -6 * 60; // minutes offset from UTC
}

function getCutoffForPayday(payday) {
  // Thursday of the week before payday at 10:00 AM CT
  ...
  // thursdayMidnight is midnight UTC on that Thursday (payday parsed as UTC midnight)
  // Add hours to reach 10:00 AM CT
  const ctOffset = getCentralOffset(thursdayMidnight); // minutes (negative)
  // 10:00 AM CT = 10:00 - ctOffset/60 in UTC hours
...
function computeBHPeriods() {
  ...
  if (cutoffPassed && nextPayday) {
    const nextIdx = BH_PAYDAYS_2026.findIndex(
      (pdStr) => new Date(`${pdStr}T00:00:00Z`).getTime() === nextPayday.getTime()
    );
  ...
  useEffect(() => {
    if (show && !isMobile && buttonRef.current && tooltipRef.current) {
      ...
      setPos({
        ...
        left: br.left - tr.width / 2 < 10 ? false : br.left + tr.width / 2 > window.innerWidth - 10 ? true : "center",
      });
    }
  }, [show, isMobile]);
...
function TogglePair({ options, value, onChange, small = false }) {
  return (
...
// Plan-specific stat card with color coding
function PlanCard({ planLabel, planColor, planBg, planBorder, children }) {
  return (
    <div style={{
      background: planBg, border: `1px solid ${planBorder}`,
      borderRadius: T.radius, padding: "12px 14px", flex: 1, minWidth: 0,
    }}>
      <div style={{ fontSize: "0.65rem", fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: planColor, fontFamily: T.font, marginBottom: 8 }}>
        {planLabel}
      </div>
      {children}
    </div>
...
function PlanLine({ label, value, bold, highlight }) {
  return (
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", padding: "3px 0", borderBottom: `1px solid rgba(0,0,0,0.06)` }}>
      <span style={{ fontSize: "0.75rem", color: T.textSub, fontFamily: T.font, fontWeight: bold ? 600 : 400 }}>{label}</span>
...
export default function App() {
  ...
  // YTD section visibility toggles
  ...
  const catchUpAmt = getCatchUp(parsedAge);
  ...
  function calculate() {
    ...
    setTimeout(() => {
      ...
      // ── Employer contributions (independent of target) ───────────────────
      ...
      // ── Target hierarchy logic ───────────────────────────────────────────
      // Budget = target minus what's already been contributed YTD across all plans
      // If no target, budget is effectively unlimited (max everything)
      ...
      let budget; // remaining employee dollars to allocate
      let usingTarget = false;
      if (useTarget && parse(targetAmount) > 0) {
        const t = parse(targetAmount);
        budget = Math.max(t - totalYtdEmployee, 0);
        usingTarget = true;
      } else {
        budget = maxEmployee - totalYtdEmployee; // max out everything
      }

      // ── Step 1: 403(b) elective ──────────────────────────────────────────
      ...
      const afterTax403bChecks = afterTax403bRemFinal > 0
        ? Math.ceil(afterTax403bRemFinal / ((perCheck * afterTax403bPct) / 100))
        : 0;
      const afterTax403bNotNeeded = usingTarget && afterTax403bAllocated === 0 && afterTax403bRemMax > 0;

      // ── Step 3: 401(a) after-tax ─────────────────────────────────────────
      ...
      const afterTax401aChecks = afterTax401aRem > 0
        ? Math.ceil(afterTax401aRem / ((perCheck * afterTax401aPct) / 100))
        : 0;
      ...
      setResult({
        ...
        // 403(b) elective
        electiveLimit, electiveRem, electivePct, electiveDpc, electiveChecks,
        electiveNotNeeded,
        ...
        afterTax403bPct, afterTax403bDpc, afterTax403bChecks,
        afterTax403bNotNeeded,
        ytd403bAfterTaxAmt,

        // 401(a)
        empMatchAmt, empDiscAmt, totalEmployer401a,
        afterTax401aLimit, afterTax401aRem,
        afterTax401aPct, afterTax401aDpc, afterTax401aChecks,
        afterTax401aNotNeeded,
        ytd401aAfterTaxAmt,
      ...
      <div style={{
        ...
        <div style={{ background: T.surface, borderRadius: T.radiusLg, border: `1px solid ${T.border}`, boxShadow: T.shadowMd, display: "flex", flexDirection: "column", overflow: "hidden" }}>
          ...
          <div style={{ overflowY: isMobile ? "visible" : "auto", padding: "12px 16px" }}>
            ...
            {/* 403(b) YTD */}
            <div
              onClick={() => { setShow403bYtd(v => { if (v) { setYtd403bPre(""); setYtd403bRoth(""); setYtd403bAfterTax(""); } markDirty(); return !v; }); }}
              style={{ display: "flex", alignItems: "center", justifyContent: "space-between", margin: "10px 0 6px", cursor: "pointer", userSelect: "none", padding: "5px 8px", borderRadius: T.radius, transition: "background 0.15s" }}
              onMouseOver={(e) => e.currentTarget.style.background = T.surfaceAlt}
              onMouseOut={(e) => e.currentTarget.style.background = "transparent"}
            >
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ fontSize: "0.82rem", fontWeight: 600, letterSpacing: "-0.01em", color: show403bYtd ? T.btn : T.textSub, fontFamily: T.font, transition: "color 0.15s" }}>403(b) YTD</span>
                {!show403bYtd && <span style={{ fontSize: "0.68rem", color: T.textMuted, fontFamily: T.font }}>Optional</span>}
              </div>
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none" style={{ flexShrink: 0, transition: "transform 0.2s", transform: show403bYtd ? "rotate(180deg)" : "rotate(0deg)" }}>
                <path d="M3 5l4 4 4-4" stroke={show403bYtd ? T.btn : T.textSub} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            </div>
            {show403bYtd && (
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 10 }} className="mobile-stack">
                <div>
            ...
            {/* 401(a) YTD */}
            <div
              onClick={() => { setShow401aYtd(v => { if (v) { setYtd401aAfterTax(""); } markDirty(); return !v; }); }}
              style={{ display: "flex", alignItems: "center", justifyContent: "space-between", margin: "4px 0 6px", cursor: "pointer", userSelect: "none", padding: "5px 8px", borderRadius: T.radius, transition: "background 0.15s" }}
              onMouseOver={(e) => e.currentTarget.style.background = T.surfaceAlt}
              onMouseOut={(e) => e.currentTarget.style.background = "transparent"}
            >
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ fontSize: "0.82rem", fontWeight: 600, letterSpacing: "-0.01em", color: show401aYtd ? T.btn : T.textSub, fontFamily: T.font, transition: "color 0.15s" }}>401(a) YTD</span>
                {!show401aYtd && <span style={{ fontSize: "0.68rem", color: T.textMuted, fontFamily: T.font }}>Optional</span>}
              </div>
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none" style={{ flexShrink: 0, transition: "transform 0.2s", transform: show401aYtd ? "rotate(180deg)" : "rotate(0deg)" }}>
                <path d="M3 5l4 4 4-4" stroke={show401aYtd ? T.btn : T.textSub} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            </div>
            {show401aYtd && (
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 10 }} className="mobile-stack">
            ...
            {/* 457(b) YTD */}
            <div
              onClick={() => { setShow457bYtd(v => { if (v) { setYtd457b(""); } markDirty(); return !v; }); }}
              style={{ display: "flex", alignItems: "center", justifyContent: "space-between", margin: "4px 0 6px", cursor: "pointer", userSelect: "none", padding: "5px 8px", borderRadius: T.radius, transition: "background 0.15s" }}
              onMouseOver={(e) => e.currentTarget.style.background = T.surfaceAlt}
              onMouseOut={(e) => e.currentTarget.style.background = "transparent"}
            >
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ fontSize: "0.82rem", fontWeight: 600, letterSpacing: "-0.01em", color: show457bYtd ? T.btn : T.textSub, fontFamily: T.font, transition: "color 0.15s" }}>457(b) YTD</span>
                {!show457bYtd && <span style={{ fontSize: "0.68rem", color: T.textMuted, fontFamily: T.font }}>Optional</span>}
              </div>
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none" style={{ flexShrink: 0, transition: "transform 0.2s", transform: show457bYtd ? "rotate(180deg)" : "rotate(0deg)" }}>
                <path d="M3 5l4 4 4-4" stroke={show457bYtd ? T.btn : T.textSub} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            </div>
            {show457bYtd && (
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 10 }} className="mobile-stack">
            ...
            {/* Contribution Goal */}
            <div
              onClick={() => { setUseTarget(v => { if (v) { setTargetAmount(""); setErrors((p) => ({ ...p, targetAmount: "" })); } markDirty(); return !v; }); }}
              style={{ display: "flex", alignItems: "center", justifyContent: "space-between", margin: "4px 0 6px", cursor: "pointer", userSelect: "none", padding: "5px 8px", borderRadius: T.radius, transition: "background 0.15s" }}
              onMouseOver={(e) => e.currentTarget.style.background = T.surfaceAlt}
              onMouseOut={(e) => e.currentTarget.style.background = "transparent"}
            >
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ fontSize: "0.82rem", fontWeight: 600, letterSpacing: "-0.01em", color: useTarget ? T.btn : T.textSub, fontFamily: T.font, transition: "color 0.15s" }}>Contribution Goal</span>
                {!useTarget && <span style={{ fontSize: "0.68rem", color: T.textMuted, fontFamily: T.font }}>Maximizes all plans by default</span>}
              </div>
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none" style={{ flexShrink: 0, transition: "transform 0.2s", transform: useTarget ? "rotate(180deg)" : "rotate(0deg)" }}>
                <path d="M3 5l4 4 4-4" stroke={useTarget ? T.btn : T.textSub} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            </div>
            {useTarget && (
              <div style={{ marginBottom: 10 }}>
                <Label tooltip={`Enter your total employee contribution goal for ${PLAN_YEAR}. The calculator will fill plans in order — 403(b) elective first, then 403(b) after-tax, then 401(a) after-tax, then 457(b) — until your target is reached. Employer contributions are not counted toward your target.`}>
          ...
          <div style={{ overflowY: (isMobile || !result) ? "hidden" : "auto", padding: "14px 16px" }}>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              {cutoffPassed && firstEligiblePayday && (
                <SummaryLine label="Next available payday" value={fmtPayday(firstEligiblePayday)} bold />
              )}
              {(cutoffPassed ? firstEligibleCutoff : cutoffDate) && (
                <SummaryLine label="Change deadline" value={fmtCutoff(cutoffPassed ? firstEligibleCutoff : cutoffDate)} />
              )}
            </div>
            ...
            {!result || isCalculating ? (
              ...
              <div>
                ...
                {/* Projected Full Year Summary */}
                {(() => {
                  const proj403bElective = result.ytd403bElective + Math.min(
                    result.electiveRem > 0 ? result.electiveDpc * result.electiveChecks : 0,
                    result.electiveRem
                  );
                  const proj403bAfterTax = result.ytd403bAfterTaxAmt + Math.min(
                    result.afterTax403bRem > 0 ? result.afterTax403bDpc * result.afterTax403bChecks : 0,
                    result.afterTax403bRem
                  );
                  const proj401aAfterTax = result.ytd401aAfterTaxAmt + Math.min(
                    result.afterTax401aRem > 0 ? result.afterTax401aDpc * result.afterTax401aChecks : 0,
                    result.afterTax401aRem
                  );
                  const proj457b = result.ytd457bAmt + Math.min(
                    result.rem457b > 0 ? result.dpc457b * result.checks457b : 0,
                    result.rem457b
                  );
                  ...
                  return (
                    <details style={{ background: "#F9FAFB", border: "1px solid #E5E7EB", borderRadius: "8px", padding: "12px", marginTop: 12 }}>
                      ...
                      <div style={{ paddingTop: 8, marginTop: 8 }}>
                        ...
                        <SummaryLine
                          label={`Match — 50% × 8% of ${fc(result.compBasis)}`}
                          value={fc(result.empMatchAmt)}
                          dimmed
                          indent
                        />
                        <SummaryLine
                          label={`Discretionary — 3% of ${fc(result.compBasis)}`}
                          value={fc(result.empDiscAmt)}
                          dimmed
                          indent
                        />
